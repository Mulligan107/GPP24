...
namespace TiledCSPlus
{
    ...
    public class TiledMap
    {
        ...
        /// </summary>
        public string MapVersion { get; internal set; }
        ...
        public void ParseXml(string xml)
        {
            try
            {
                ...
                MapVersion = nodeMap.Attributes["version"].Value;
        ...
        private TiledMapTileset[] ParseTilesets(XmlNodeList nodeList)
        {
            ...
            foreach(XmlNode node in nodeList)
            {
                if(node.Attributes["source"] == null)
                {
                    ...
                    {
                        FirstGid = int.Parse(node.Attributes["firstgid"].Value), IsTilesetEmbedded = true
                    };
        ...
        private TiledGroup[] ParseGroups(XmlNodeList nodeListGroups)
        {
            ...
            foreach(XmlNode node in nodeListGroups)
            {
                ...
                {
                    Id = int.Parse(node.Attributes["id"].Value), Name = node.Attributes["name"].Value
                };
        ...
        private void ParseTileLayerData(XmlNode nodeData, ref TiledLayer tiledLayer)
        {
            ...
            if(Infinite)
            {
                ...
                foreach(XmlNode nodeChunk in nodesChunk)
                {
                    ...
                    if(encoding == "csv")
                        ParseTileLayerDataAsCSV(nodeChunk.InnerText, out chunkData, out chunkDataRotationFlags);
                    if(encoding == "base64")
                        ParseTileLayerDataAsBase64(nodeChunk.InnerText, compression, out chunkData,
                            out chunkDataRotationFlags);
            ...
            else
            {
                ...
                byte[] chunkDataRotationFlags = new byte[] { };
                if(encoding == "csv")
                    ParseTileLayerDataAsCSV(nodeData.InnerText, out chunkData, out chunkDataRotationFlags);
                if(encoding == "base64")
                    ParseTileLayerDataAsBase64(nodeData.InnerText, compression, out chunkData,
                        out chunkDataRotationFlags);
        ...
        private void ParseTileLayerDataAsCSV(string input, out int[] data, out byte[] dataRotationFlags)
        {
            ...
            for(var i = 0; i < csvs.Length; i++)
            {
                ...
                // assign data to rawID with the rotation flags cleared
                data[i] =
                    (int)(rawID & ~(FLIPPED_HORIZONTALLY_FLAG | FLIPPED_VERTICALLY_FLAG | FLIPPED_DIAGONALLY_FLAG));
            }
        }
