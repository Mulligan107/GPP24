...
namespace ShooterGame
{
    ...
    {
        public GameOverMenu(IntPtr renderer) : base(renderer)
        {
            var menuItemSpacing = Program.SCREEN_HEIGHT / 3; // Divide by the number of menu items + 1
            var itemWidth = 200;

            var retryItem = new MenuItem("Retry", () =>
                {
                    Program.reset = true;
        ...
        public override void Render(IntPtr renderer)
        {
            ...
            DisplayText("Game Over", titlePosition, 500, "lazy.ttf", renderer, titleColor); // Adjust the width as needed
